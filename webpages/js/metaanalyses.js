(function (window, document) { // eslint-disable-line no-unused-vars
  ...
  function populateAddColumnsList(columns) {
    ...
    Object.keys(columns).forEach(function(colId) {
      ...
      if (col.formula) {
        if (metaanalysisContainsAllFormulaColumns(currentMetaanalysis.columnOrder, col.formulaColumns)) {
  ...
  function metaanalysisContainsAllFormulaColumns(metaanalysisColumnOrder, formulaColumns) {
    // TODO? Formula columns might be undefined, if so do we can show this column?
    if (!formulaColumns) return false;
    if (!metaanalysisColumnOrder) return false;

    for (var i = 0; i < formulaColumns.length; i++) {
        if (metaanalysisColumnOrder.indexOf(formulaColumns[i]) == -1) return false;
    }
    return true;
  }
